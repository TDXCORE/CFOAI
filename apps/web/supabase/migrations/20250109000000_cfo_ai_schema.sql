...
-- Create a private schema for CFO AI functions
...
-- Municipal ICA rates
CREATE TABLE IF NOT EXISTS ica_rates (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE IF NOT EXISTS accounts_mapping (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_invoices_supplier ON invoices(tenant_id, supplier_nit);
CREATE INDEX IF NOT EXISTS idx_invoices_status ON invoices(tenant_id, status);
CREATE INDEX IF NOT EXISTS idx_invoices_needs_review ON invoices(tenant_id, needs_review) WHERE needs_review = TRUE;

-- Processing indexes
CREATE INDEX IF NOT EXISTS idx_processing_jobs_status ON processing_jobs(tenant_id, status, created_at);
CREATE INDEX IF NOT EXISTS idx_processing_jobs_queue ON processing_jobs(status, priority DESC, created_at) WHERE status IN ('queued', 'parsing');

-- Files and messages
CREATE INDEX IF NOT EXISTS idx_files_tenant_source ON files(tenant_id, source, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_mail_messages_tenant_status ON mail_messages(tenant_id, status, received_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_classifications_invoice ON classifications(invoice_id);
...
CREATE OR REPLACE FUNCTION cfo_ai.user_tenant_ids()
RETURNS UUID[] AS $$
...
GRANT EXECUTE ON FUNCTION cfo_ai.user_tenant_ids() TO authenticated, service_role;
...
CREATE OR REPLACE FUNCTION cfo_ai.update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER update_tenants_updated_at 
    ...
    FOR EACH ROW 
    EXECUTE PROCEDURE cfo_ai.update_updated_at_column();
...
VALUES 
  ('invoices', 'invoices', false, 52428800, ARRAY['application/xml', 'application/pdf', 'image/jpeg', 'image/png', 'application/zip']),
...
FOR ALL USING (
  ...
  (storage.foldername(name))[1] IN (
    SELECT tenant_id::text FROM user_tenants WHERE user_id = auth.uid() AND status = 'active'
  )
);
